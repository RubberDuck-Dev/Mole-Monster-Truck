...
const MONSTER_TRUCK_DRIVEN = preload("uid://645nsjlb1x5f")
...
func _ready() -> void:
	...
	if can_spawn_mole:
		spawn_mole()
	if can_spawn_wheel and not GameManager.has_part(GameManager.current_idx):
		spawn_wheel()
...
#				AudioManager.play_sfx("caught")
	...
	if GameManager.stage_completed:
...
func _collect_part() -> void:
	if wheel_instance == null:
		return
	GameManager.collect_part()
	AudioManager.play_sfx("collected")
	HUD.start_dialogue(4)
	wheel_instance.queue_free()
	wheel_instance = null

	if has_node("LeftExitLight"):
	...
	if has_node("BackgroundArt/MonsterTruck"):
		...
		match wheels:
			0:
				pass
			1:
				$BackgroundArt/MonsterTruck/Wheel1.visible=true
			2:
				$BackgroundArt/MonsterTruck/Wheel1.visible=true
				$BackgroundArt/MonsterTruck/Wheel2.visible=true
			_:
				$BackgroundArt/MonsterTruck/Wheel1.visible=true
				$BackgroundArt/MonsterTruck/Wheel2.visible=true

func ride_truck()->void:
	#cinematic
	if has_node("BackgroundArt/MoleBed"):
		$BackgroundArt/MoleBed.visible=false
		mole_instance.can_move=false
